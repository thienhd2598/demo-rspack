...
const TableWarehouseBillOut = ({ onSetSchema, warehouse, typeProduct, selectedVariants, setSelectedVariants}) => {
    const { formatMessage } = useIntl();
    const { addToast } = useToasts();
    const history = useHistory();
    const location = useLocation();
    const params = queryString.parse(useLocation().search.slice(1, 100000));
    const { validateForm, handleSubmit, setFieldValue, values, setValues } = useFormikContext();
