...
const TableWarehouseBillIn = ({ onSetSchema, status, warehouse, expense, setExpense, generalData }) => {
    ...
    const onUpdateWareHouseBill = async (status) => {
        ...
        if (data?.warehouseApproveBill?.success) {
            ...
            addToast(
                data?.warehouseApproveBill?.message || formatMessage({ defaultMessage: `Duyệt phiếu nhập kho thất bại` }), { appearance: 'error' }
            );
        }
    }
                                    ...
                                    !error && !loading && data?.warehouse_bill_items?.map(_wareHouseBill => {
                                        ...
                                        return (
                                            <tr key={`warehouse-bill-item-${_wareHouseBill?.id}`} style={{ borderBottom: '1px solid #D9D9D9' }}>
                                                ...
                                                {status == 'new' && <td className="text-center">
