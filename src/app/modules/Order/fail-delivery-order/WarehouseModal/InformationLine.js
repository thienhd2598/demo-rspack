...
export default function InformationLine({ dataStore, order, dataChannels }) {
  const [isCopied, setIsCopied] = useState(false);
  const { formatMessage } = useIntl();

  const [logoStore, nameStore] = useMemo(() => {
    const findedStore = dataStore?.sc_stores?.find(st => st?.id == order?.store_id);
    const findedChannel = dataChannels?.find(cn => cn?.code == order?.connector_channel_code);

    return [
      findedChannel?.logo_asset_url,
      findedStore?.name
    ]
  }, [order, dataStore, dataChannels]);
      ...
      <td colSpan="7" className="p-0">
        ...
        >
          <div className="d-flex align-items-center">
            ...
            >
              <span
                onClick={() => onCopyToClipBoard(order?.ref_id)}
