...
import { STATUS_FAIL_DELIVERY_ORDER, STATUS_ORDER } from "../OrderUIHelpers";
import dayjs from "dayjs";
import _ from "lodash";
import mutate_scExportFailDeliveryOrder from "../../../../graphql/mutate_scExportFailDeliveryOrder";
import { useToasts } from "react-toast-notifications";
import query_scExportFailDeliveryOrderAggregate from "../../../../graphql/query_scExportFailDeliveryOrderAggregate";
...
function ExportDialog({ show, onHide, params }) {
  ...
  const { data: dataOrderAggregate, loading: loadingOrderAggregate } = useQuery(query_scExportFailDeliveryOrderAggregate, {
    variables: {
      ..._whereCondition
    }
  });
  ...
  const [mutate, { loading, data }] = useMutation(mutate_scExportFailDeliveryOrder)
      ...
      <Modal.Body className="overlay overlay-block cursor-default" >
        ...
        <div className="col-12" >
          ...
          <div className="row mt-3 display-flex align-items-center " >
            ...
            <div className="col-8" >
              <Select options={STATUS_FAIL_DELIVERY_ORDER.slice(0, 100).map((_status, _idx) => {
                return {
                  ..._status,
                  label: _status.title,
                  value: _idx
          ...
          <div className="row mt-3 display-flex align-items-center " >
            <div className="col-4" >{formatMessage({ defaultMessage: 'Thời gian trở về' })}</div>
          ...
          {
            !!dataOrderAggregate?.scExportFailDeliveryOrderAggregate && <div className="row mt-3 display-flex align-items-center " >
              <div className="col-4" >{formatMessage({ defaultMessage: 'Số lượng đơn hàng' })}</div>
              <div className="col-8" >
                <strong>{dataOrderAggregate?.scExportFailDeliveryOrderAggregate?.count || '0'}</strong> {formatMessage({ defaultMessage: 'đơn' })}
              </div>
            </div>
          }
        </div>
      ...
      <Modal.Footer className="form" style={{ borderTop: 'none', justifyContent: 'center', paddingTop: 0 }} >
        <div className="form-group">
          ...
          <button
            ...
            style={{ width: 100 }}
            disabled={!_whereCondition || !dataOrderAggregate?.scExportFailDeliveryOrderAggregate?.count || loading}
            onClick={async e => {
              e.preventDefault()
              try {
                ...
                if (data?.scExportFailDeliveryOrder?.success == 1) {
                  ...
                  addToast(data?.scExportFailDeliveryOrder?.message || formatMessage({ defaultMessage: "Gửi yêu cầu xuất đơn hàng không thành công" }), { appearance: 'error' });
                }
              } catch (error) {
