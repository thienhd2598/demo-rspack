...
const WarehouseModalDetail = memo(
  ({ dataStore, dataChannels, orderProcess, onHide }) => {    
    ...
    console.log('productVideFiles', productVideFiles)
    const { data: dataWarehouse } = useQuery(query_sme_catalog_stores, {
      fetchPolicy: "cache-and-network",
    });
    const orderInreturnOrderItems = orderProcess?.orderItems;
    ...
    useMemo(() => {
      ...
      }, [orderProcess.returnWarehouseImport]
    );
    ...
    const caculateImportQuantity = (id, order_item_id) => {
      ...
      const findedImportItem = orderProcess?.returnWarehouseImport?.returnWarehouseImportItems?.find(item => item?.sme_variant_id === id && item?.cancelOrderItem?.id === order_item_id);
    ...
    useMemo(async () => {
      ...
      const returnImportIds = orderProcess?.returnWarehouseImport?.returnWarehouseImportItems?.map(_item => {
        return [_item?.sme_combo_variant_id, _item?.sme_variant_id]
      });
     ...
     const totalVariants = [...variants, ...totalParentVariants]
      ...
      const rebuild = orderInreturnOrderItems?.map((_item, index) => {
        let productVariant;
        const returnItem = orderProcess?.orderItems?.find((_ro) => _ro?.id === _item?.id);
        const filteredReturnImport = orderProcess?.returnWarehouseImport?.returnWarehouseImportItems?.filter(item => item?.cancelOrderItem?.id == _item?.id);  

        const isComboImport = filteredReturnImport?.every(item => !!item?.sme_combo_variant_id);

        console.log({ filteredReturnImport })
        ...
        if (_item?.sme_variant_id) {
          ...
          productVariant = {
            ...
            return_quantity: caculateReturnQuantity(_item?.sme_variant_id, _item?.id) || returnItem?.quantity_purchased,
            ...
            combo_items: _item?.comboItems?.map(_combo => {
              ...
              return {
                ...
                return_quantity: caculateReturnQuantity(_combo?.sme_variant_id, _combo?.order_item_id) || _combo?.purchased_quantity,
          ...
          if (filteredReturnImport?.length > 0) {
            // Check hàng hóa import được chọn là combo || thường
            if (isComboImport) {
              const findedVariant = totalVariants?.find(variant => variant?.id === filteredReturnImport?.[0]?.sme_combo_variant_id);              

              productVariant = {
                ...findedVariant,
                import_quantity: caculateImportQuantity(filteredReturnImport?.[0]?.sme_combo_variant_id, _item?.id),
                return_quantity: caculateReturnQuantity(filteredReturnImport?.[0]?.sme_combo_variant_id, _item?.id) || returnItem?.quantity_purchased,
                is_combo: true,
                combo_items_origin: findedVariant?.combo_items,
                combo_items: findedVariant?.combo_items?.map(_combo => {
                  const findedVariantCombo = filteredReturnImport?.find(item => item?.sme_variant_id == _combo?.combo_variant_id)
              ...
              const findImported = orderProcess?.returnWarehouseImport?.returnWarehouseImportItems?.find(item => item?.cancelOrderItem?.id == _item?.id)
              let findedVariant = {}
        ...
        return {
          ...
          quantityReturn: returnItem?.quantity_purchased,
