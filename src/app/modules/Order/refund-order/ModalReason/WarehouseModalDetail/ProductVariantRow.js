...
const ProductVariantRow = ({
  ...
  const returnItemsWitoutCombo = returnItems?.filter(item => item?.returnOrderItem?.order_item_id == order?.id && !item?.sme_combo_variant_id)
  ...
  const buildVariant = useMemo(() => {
    ...
    // Case có liên kết
    const returnItem = returnItemsWitoutCombo?.find(importItem => importItem?.returnOrderItem?.order_item_id == order?.id && order?.productVariant?.id == importItem?.sme_variant_id)
      ...
      <>
        ...
        {(listStatus?.length == 1 ? !!(listStatus[0]?.id != variantView?.id) : !!listStatus?.length) ? (
          ...
          <>
            ...
            <td className="text-center" style={{ verticalAlign: "top" }}>{variantView?.product_status_name}</td>
            <td className="text-center" style={{ verticalAlign: "top", borderRight: '1px solid #d9d9d9' }}>
              {`${returnItem?.import_quantity|| 0} / ${order?.quantityReturn || 0}`}
            </td>
          </>
        )}
      </>
    ...
    return (
      ...
      <tr>
        ...
        {order.productVariant?.combo_items?.map(_combo => {
            // Lấy toàn bộ trạng thái hàng hóa của item trong combo
            const comboListStatus = [{..._combo?.combo_item},..._combo?.combo_item?.status_variants]?.filter(item => !!item?.status)
            // Lấy return order đã import của combo
            const returnItemsInCombo = returnItems?.filter(item => item?.returnOrderItem?.order_item_id == order?.id && item?.sme_combo_variant_id == order.productVariant?.id)
            // Lấy những trạng thái của combo item đã được nhập kho
            const listStatusCombo = comboListStatus?.filter(status => returnItemsInCombo?.map(item => item?.sme_variant_id)?.includes(status?.id))
            // Tìm kiếm combo item đã được nhập kho chưa
            ...
            <tr>
              ...
              {(listStatusCombo?.length == 1 ? !!(listStatusCombo[0]?.id != _combo?.combo_item?.id) : !!listStatusCombo?.length) ? (
                ...
                <>
                  <td className="text-center" style={{ verticalAlign: "top", borderBottom: '1px solid #d9d9d9', borderTop: '1px solid #d9d9d9' }}>{currentStatusCombo?.unit || _combo?.combo_item?.unit || '--'}</td>
                  <td className="text-center" style={{ verticalAlign: "top" , borderBottom: '1px solid #d9d9d9', borderTop: '1px solid #d9d9d9'}}>{currentStatusCombo?.product_status_name || _combo?.combo_item?.product_status_name}</td>
  ...
  return (
    <>
      ...
      {(listStatus?.length == 1 ? !!(listStatus[0]?.id != variantView?.id) : listStatus?.length > 1) && isLinked && listStatus?.map(item => {

        const quantityImport = returnItemsWitoutCombo?.find(importItem => importItem?.returnOrderItem?.order_item_id == order?.id && item?.id == importItem?.sme_variant_id)
