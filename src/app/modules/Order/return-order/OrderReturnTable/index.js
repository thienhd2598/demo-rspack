...
const OrderReturnTable = memo(({
  ...
  const [typeSub, setTypeSub] = useState("PROCESSING");
  useMemo(
    () => {
      if (!params.status) {
  ...
  return (
    <div style={{
      ...
      <div
        style={{ position: "sticky", top: 107, background: "#fff", zIndex: 1 }}
      >
      ...
      <div
        ...
        className="mb-2 mr-4 ml-2 text-primary"
      >
        {formatMessage({defaultMessage: 'Đã chọn'})}: {ids.length || 0} {formatMessage({defaultMessage: 'sản phẩm'})}
      </div>
    ) : null} */}
      {/* //table */}
      ...
      {!error && (
        <Pagination
          ...
          count={data?.length}
          basePath={"/orders/return-order"}
          emptyTitle="Không tìm thấy đơn hàng phù hợp"
        />
      )}
    </div>
  )
