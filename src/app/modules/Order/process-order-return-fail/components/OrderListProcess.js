...
import Pagination from "../../process-order-return/view/Pagination";
...
const OrderListProcess = ({ values, loadingScan, state, updateState }, ref) => {
  ...
  let totalPage = Math.ceil(totalRecord / onSizePage);
  const count = dataTable?.slice(
    onSizePage * (page - 1),
    onSizePage + onSizePage * (page - 1)
  ).length
  useEffect(() => {
    setDataTable(state.dataScaned);
  }, [state.dataScaned]);
  const value_select_search = OPTIONS_SELECT?.find(
    (__item) => __item.value == state.typeOptionSearch
  );

  const placeholderInputSearch = `Tìm ${formatMessage(
    value_select_search.label
  ).toLocaleLowerCase()}`;
  ...
  const handleSearch = useCallback(() => {
    try {
      const dataScanFilltred = state.dataScaned.filter((__order) => {
        const { other } = __order;
        if (
          other[state.typeOptionSearch].toLowerCase().includes(
  ...
  const isOrderManual = useMemo(() => {
    if (dataTable?.length == 0) return false;

    return dataTable[0]?.other?.source == 'manual';
  }, [dataTable]);
    ...
    <Card>
      <CardBody>
        ...
        <div className="form-group my-4 d-flex align-items-center">
          ...
          <div
            className="col-7 input-icon pl-0 ml-0"
        ...
        <div className="warning__title mb-2 d-flex align-items-center">
          ...
          <span className="text-danger fs-14">
            {formatMessage({
              defaultMessage:
                "Chú ý: Khi đã xử lý nhập kho, thì không thể huỷ nhập kho cho đơn đã xử lý",
            })}
          </span>
        </div>
        ...
        <div>
          <table
            ...
            className="table table-borderless table-vertical-center fixed mb-0"
          >
            <thead
              style={{
                background: "#F3F6F9",
        ...
        <Pagination
          ...
          emptyTitle={formatMessage({
            defaultMessage: "Không tìm thấy đơn hàng phù hợp",
          })}
        />
      </CardBody>
    </Card>
