...
import mutate_coValidateExcelImportWarehouse_cancel from "../../../../../graphql/mutate_coValidateExcelImportWarehouse_cancel";
...
const ModalUploadFile = ({
  ...
  const [coValidateExcelImportWarehouse] = useMutation(
    mutate_coValidateExcelImportWarehouse_cancel,
    {
    ...
    if (data?.coValidateExcelImportWarehouse?.success == 1) {
      ...
      addToast(
        data?.coValidateExcelImportWarehouse?.message ||
          formatMessage({
            defaultMessage: "Nhập file đơn hủy không thành công",
          }),
        { appearance: "error" }
      );
    }
  ...
  return (
    ...
    >
      <Modal.Header closeButton={false}>
        <Modal.Title>
          {formatMessage({ defaultMessage: "Nhập file đơn huỷ bất thường" })}
        </Modal.Title>
          <span>
          ...
          <div className="col-9 px-0 mb-5">
            <button
              ...
              onClick={async () => {
                ...
                saveAs(fileTemplate, dowloadName);
            }}
            >
