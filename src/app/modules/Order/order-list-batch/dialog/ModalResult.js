    ...
    return (
        ...
        >
            ...
            <Modal.Body className="overlay overlay-block cursor-default">
                ...
                />
                <div className='row'>
                    <div className='col-12'>{formatMessage({defaultMessage: 'Kiện hàng đã chọn'})}: <span className='font-weight-bold'>{(dataResults?.total_success + dataResults?.total_fail) ?? 0}</span></div>
                    ...
                    <div className='col-12 mt-3'>{formatMessage({defaultMessage: 'Kiện hàng xử lý thất bại'})}: <span className='font-weight-bold text-danger'>{dataResults?.total_fail ?? 0}</span></div>
                    {dataResults?.list_package_fail?.length > 0 && <div className='col-12 mt-4'>
                        <table className="table table-borderless product-list table-vertical-center fixed">
                            ...
                            <tbody>
                                ...
                                {dataResults?.list_package_fail?.slice(5 * (page - 1), 5 + 5 * (page - 1))?.map((data) => {
                        ...
                        {type === 'pack-prepare' && 
                             <p className='text-secondary-custom fs-12 mt-2'>
                                {formatMessage({defaultMessage: 'Chú ý: Những kiện hàng thất bại có thể xem ở mục “ Chuẩn bị hàng lỗi”'})}
                            </p>
                        }
                        <div
                        className="col-12" style={{padding: "1rem", boxShadow: "rgb(0 0 0 / 20%) 0px -2px 2px -2px" }}>
                            <PaginationModal
                                page={page}
                                totalPage={totalPage}
                                limit={5}
