...
const InfoProcess = (
  ...
  const handleScan = (e) => {
    if (e.keyCode == 13) {
      ...
      if (state.dataScaned.length == MAX_SCAN_ORDER) {
        inputRefOrder.current.value = "";
        addToast(
          formatMessage({ defaultMessage: "Hiện tại, hệ thống chỉ xử lý 200 đơn/lần, vui lòng thêm đơn hoàn mới tại lượt xử lý kế tiếp" }),
          { appearance: "error" }
        );
        return;
      }
