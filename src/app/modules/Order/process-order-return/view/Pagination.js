...
export default memo(
  ({
    ...
    setPage,
    basePath,
    emptyTitle = useIntl().formatMessage({
              ...
              {loading && (
                <div className="d-flex align-items-center">
                  <div className="mr-2 text-muted">
                    {formatMessage({ defaultMessage: "Đang tải" })}...
                  </div>
                  <div className="spinner spinner-success mr-10"></div>
                </div>
              )}
              <span className="text-muted">
                {formatMessage({ defaultMessage: "Hiển thị 0 - 0 của 0" })}
              </span>
            </div>
          </div>
        </>
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap">
        <div className="d-flex align-items-center py-3 px-3 col-6">
            ...
            >
              {optionsPagination?.map((_option) => (
                <option
                  ...
                  value={_option.value}
                >
                  {`${_option.label} ${formatMessage({
                    defaultMessage: "bản ghi/trang",
                  })}`}
                </option>
              ))}
            </select>
          ...
          <span className="text-muted col-8">
            {formatMessage({ defaultMessage: "Hiển thị" })}{" "}
            {(page - 1) * limit + 1} - {(page - 1) * limit + count}{" "}
            {!!totalRecord ? `của ${totalRecord}` : ""}
          </span>
        </div>
