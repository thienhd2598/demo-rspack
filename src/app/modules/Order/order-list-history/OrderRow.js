...
export default memo(({ onSetSmeNote, params, order: orderPack, key,
    op_connector_channels, sc_stores,
    dataSmeWarehouse, dataScWareHouse,
                        ...
                        {tts_expired ? tts_expired - dayjs().unix() > 0 ?
                            !!orderPack?.order?.expiring_soon ?
                                <div style={{ color: 'F80D0D' }}>
                                    ...
                                    <TooltipWrapper note={<div><span>{formatMessage({ defaultMessage: 'Còn lại' })}: </span>{Math.floor((tts_expired - dayjs().unix()) / 3600)}
                ...
                {<div className='d-flex mt-2'>
                    {
                        ...
                        <OverlayTrigger
                            overlay={
                                <Tooltip>
                                    {formatMessage({ defaultMessage: 'Đã in vận đơn' })}
                                </Tooltip>
                            }
                        >
                    ...
                    {
                        (2 & orderPack?.print_status) == 2 && <OverlayTrigger
                            overlay={
                                <Tooltip>
                                    {formatMessage({ defaultMessage: 'Đã in phiếu xuất kho' })}
                                </Tooltip>
                            }
                        >
                    ...
                    {
                        (4 & orderPack?.print_status) == 4 && <OverlayTrigger
                            overlay={
                                <Tooltip>
                                    {formatMessage({ defaultMessage: 'Đã in phiếu bàn giao' })}
                                </Tooltip>
                            }
                        >
    ...
    const renderAction = useMemo(() => {
        return (
            <Dropdown drop='down' >
                ...
                <Dropdown.Menu>
                    {(orderPack?.order?.source != 'manual' || (pack_status != 'pending' && orderPack?.order?.source == 'manual')) &&
                ...
                <td colSpan='7' className='p-0'>
                    <div className='d-flex align-items-center justify-content-between' style={{ background: '#D9D9D9', padding: '8px' }}>
                        <div className='d-flex align-items-center'>
                            <span className='mx-4'>
                                {!!_channel?.logo_asset_url && <img src={_channel?.logo_asset_url} className='mr-1' alt="" style={{ width: 20, height: 20, objectFit: 'contain' }} />}
                            ...
                            <span
                                ...
                                onClick={() => {
                                    if (orderPack?.order?.status == 'pending' && orderPack?.order?.source == 'manual') return;
                                    window.open(`/orders/${orderPack?.order?.id}`, "_blank")
                                }}
                            >
                        ...
                        <div className='d-flex align-items-center'>
                            {!!orderPack?.order?.sme_note && (
                                <div style={{ cursor: 'pointer' }} onClick={() => onSetSmeNote(true, !['pending'].includes(orderPack?.order?.status))} className='mx-2'>
                ...
                <td style={{ verticalAlign: 'top' }} className="p-0">
                    {orderPack?.orderItems?.slice(0, (canExpand && !isExpand) ? 3 : orderPack?.orderItems?.length)
                        ?.map((item, index) => {
                            ...
                            return (
                                <Fragment>
                                    {orderPack?.order?.source == 'platform' && <OrderProductRow
