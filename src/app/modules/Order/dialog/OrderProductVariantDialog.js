...
import mutate_coLinkComboItem from '../../../../graphql/mutate_coLinkComboItem';
import { useIntl } from 'react-intl';
const OrderProductVariantDialog = memo(({
    show,
    onHide,
    scVariantSku,
    order_item_id,
    ...
    const [linkProductVariant, { loading: loadingLinkProductVariant }] = useMutation(mutate_coLinkSmeProductOrder, {
        awaitRefetchQueries: true,
        refetchQueries: ['findOrderDetail']
    });

    const [linkComboItem, { loading: loadingLinkComboItem }] = useMutation(mutate_coLinkComboItem, {
        awaitRefetchQueries: true,
        refetchQueries: ['findOrderDetail']
    });
    ...
    const onLinkProductVariant = useCallback(
        async (smeVariantId) => {
            if (!show) return;
            if (comboItemId) {
                const existedCombo = currentLstExist?.some(combo => combo === smeVariantId);

                if (existedCombo) {
                    addToast(formatMessage({ defaultMessage: 'Hàng hóa kho đã tồn tại trong combo, vui lòng chọn hàng hóa khác để thực hiện liên kết' }), { appearance: 'error' });
                    return;
                }

                let res = await linkComboItem({
                    variables: {
                ...
                if (res?.data?.coLinkComboItem?.success) {
                    onHide();
                    addToast(formatMessage({ defaultMessage: 'Liên kết đơn hàng với hàng hóa kho thành công' }), { appearance: 'success' });
                } else {
                    setError(res?.data?.coLinkComboItem?.message || 'error')
                }
            } else {
                ...
                if (res?.data?.coLinkSmeProductOrder?.success) {
                    ...
                    setError(res?.data?.coLinkSmeProductOrder?.message || 'error')
                }
            }
        }, [show, order_item_id, comboItemId, currentLstExist]
    );
    ...
    return (
        <Modal
            ...
            centered
            onHide={onHide}
            backdrop={(loadingLinkProductVariant || loadingLinkComboItem) ? 'static' : true}
            dialogClassName={(loadingLinkProductVariant || loadingLinkComboItem) ? 'width-fit-content' : 'body-dialog-connect'}
        >
            <Modal.Body className="overlay overlay-block cursor-default" style={(loadingLinkProductVariant || loadingLinkComboItem) || !!error ? {} : { padding: 0 }}>
                {
                    (loadingLinkProductVariant || loadingLinkComboItem) && <div className='text-center'>
                ...
                {
                    !loadingLinkProductVariant && !loadingLinkComboItem && !error && (
                        <Formik
                            initialValues={{
                                ...
                                <form onSubmit={handleSubmit} className="form form-label-right">
                                    <div className="form-group mb-0">
                                        <div className="text-center" style={{ fontSize: 18, fontWeight: 'bold', margin: '1rem 2rem 1rem 2rem' }}>
                                            {formatMessage({ defaultMessage: 'LIÊN KẾT HÀNG HÓA KHO VỚI ĐƠN HÀNG' })}
                                        </div>
                                        <i
                                        ...
                                        {
                                            (loadingLinkProductVariant || loadingLinkComboItem) && <div className='text-center w-100 mt-4' style={{ position: 'absolute' }} >
...
export default OrderProductVariantDialog;
