...
import query_scGetPackagesFFM from "../../../../../graphql/query_scGetPackagesFFM";
import { OPTIONS_ORDER_BY } from "../OrderFulfillmentHelper";
...
const SectionTable = () => {
    ...
    const { filtersPackage, ids, setIds, isLoadPackages, setIsLoadPackages, dataScWareHouse, optionsChannel, optionsStore, optionsSmeWarehouse, searchParams, setSearchParams } = useOrderFulfillmentContext();
    ...
    useMemo(() => {
        ...
        if (!!values?.smeWarehouse) {
            search = { ...search, warehouse_filer: 2, warehouse_id: values?.smeWarehouse }
        }
    ...
    const { data, loading, error, refetch } = useQuery(query_scGetPackagesFFM, {
        variables: {
