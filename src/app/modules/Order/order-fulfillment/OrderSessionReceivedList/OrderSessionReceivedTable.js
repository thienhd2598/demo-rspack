...
import { STATUS_SESSION_DELIVERY, TABS_STATUS_SESSION_DELIVERY } from "../OrderFulfillmentHelper";

const OrderSessionReceivedTable = ({ ids, setIds, loading, data, limit, page, error, params, dataSfCountSessionReceived, setCurrentAction, onPrintReceived }) => {
    ...
    const tabsOrderFulfillment = useMemo(() => {
        if (!dataSfCountSessionReceived?.sfCountSessionReceived) return TABS_STATUS_SESSION_DELIVERY;
        const { total, total_new, total_completed, total_cancelled } = dataSfCountSessionReceived?.sfCountSessionReceived || {}
    ...
    }, [dataSfCountSessionReceived, TABS_STATUS_SESSION_DELIVERY]);
    ...
    const columns = [
            ...
            render: (_item, record) => {
                return <div className="d-flex align-items-center">
                    ...
                    <span
                        ...
                        style={{ color: '#0962f3' }}
                        onClick={() => window.open(`/orders/session-received/${record?.id}`, '_blank')}
                    >
                        {record?.code}
                    </span>
                    <OverlayTrigger
        ...
        },
        {
            ...
            width: '10%',
            render: (_item, record) => {
                return <span>{dayjs(record?.updated_at).format('DD/MM/YYYY HH:mm')}</span>
            }
        },
        {
            title: formatMessage({ defaultMessage: 'Thời gian nhận' }),
            dataIndex: 'sku',
            key: 'sku',
            align: 'center',
            width: '10%',
            render: (_item, record) => {
                return <span>{!!record?.received_at ? dayjs.unix(record?.received_at).format('DD/MM/YYYY HH:mm') : '--'}</span>
            }
        },
        {
            ...
            render: (_item, record) => {
                ...
                return <div className="d-flex justify-content-center align-items-center">
                    <span>{record?.count_imported || 0}</span>
                        ...
                        <Dropdown drop='down' >
                            ...
                            <Dropdown.Menu>
                                ...
                                {record?.status == 1 && <Dropdown.Item
                                    ...
                                    {formatMessage({ defaultMessage: 'Hủy phiên' })}
                                </Dropdown.Item>}
                                <Dropdown.Item
                                    className="mb-1 d-flex"
                                    onClick={async e => {
                                        e.preventDefault();
                                        window.open(`/orders/session-received/${record?.id}`, '_blank');
                                    }}>
        ...
        <div className='d-flex justify-content-between align-items-center mb-4' style={{ position: 'sticky', top: 45, background: '#fff', zIndex: 3, minHeight: 37.5 }}>
            {(params?.status == 1 || params?.status == 3) ? <div className='d-flex align-items-center' style={{ minHeight: 37.5 }}>
                ...
                <AuthorizationWrapper keys={['order_session_handover_actions']}>
                    <Dropdown drop='down' style={{ zIndex: 10 }}>
                        ...
                        <Dropdown.Menu>
                            {(params?.status == 1 || params?.status == 3) && <Dropdown.Item onClick={() => onPrintReceived()} className="d-flex">
                                {formatMessage({ defaultMessage: "In biên bản" })}
                            </Dropdown.Item>}
            ...
            <AuthorizationWrapper keys={['order_session_handover_create']}>
                <button
                    ...
                    style={{ minWidth: 120 }}
                    onClick={() => history.push('/orders/session-received/create')}
                >
...
export default memo(OrderSessionReceivedTable);