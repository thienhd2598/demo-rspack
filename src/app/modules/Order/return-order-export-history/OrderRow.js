...
import mutate_scRetryExportReturnOrder from '../../../../graphql/mutate_scRetryExportReturnOrder';
...
export default memo(({ params, order, op_connector_channels }) => {
    ...
    const [mutate, { loading, data }] = useMutation(mutate_scRetryExportReturnOrder)
        ...
        <Fragment>
            <tr>
                ...
                <td >
                    ...
                    }}><AssignmentReturned /> {formatMessage({defaultMessage:'Tải file'})}</span> : (status == 2 ? <span className='cursor-pointer' onClick={async e => {
                        e.preventDefault()
                        e.stopPropagation()
                        try {
                            ...
                            if (data?.scRetryExportReturnOrder?.success == 1) {
                                addToast(formatMessage({defaultMessage:'Gửi yêu cầu xuất đơn hàng thành công'}), { appearance: 'success' });
                                refetchTracking()
                            } else {
                                addToast(data?.scRetryExportReturnOrder?.message || formatMessage({defaultMessage:"Gửi yêu cầu xuất đơn hàng không thành công"}), { appearance: 'error' });
                            }
                        } catch (error) {
