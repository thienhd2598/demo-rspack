...
export default function CreateMember() {
    ...
    useEffect(() => {
        ...
        if (!route?.params?.id) {
            ...
            appendBreadcrumbs({
                title: formatMessage({defaultMessage:'Sửa thông tin tài khoản'}),
                pathname: route.url
            })
        }
    }, [])
                                ...
                                <div className='row' >
                                    <div className='col-sm-6 col-lg-4' >
                                        ...
                                        <Field
                                            ...
                                            type='number'
                                            required={true}
                                            label={formatMessage({defaultMessage:'Nhóm quyền'})}
                                            disabled={loading || loadingUpdate}
                                            options={[]}
                                        />
                                        <div className='mt-8' />
                                ...
                                <div className='row' >
                                    <div className='col-lg-9' >
                                        ...
                                        <button type="submit" className="btn btn-primary" style={{ float: 'right', width: 170, height: 38 }}
                                            disabled={loading}
                                            onClick={handleSubmit}>{loading || loadingUpdate ? <span className="spinner spinner-white mr-4"></span> : (!route?.params?.id ? formatMessage({defaultMessage:'Tạo tài khoản'}) : formatMessage({defaultMessage:'Cập Nhật'}))}</button>

                                        <Link to='/setting/profile/members' className="btn btn-secondary mr-4" style={{ float: 'right', width: 170, height: 38 }}>{formatMessage({defaultMessage:'Huỷ bỏ'})}</Link>
                                    </div>
                                </div>
                            </CardBody>
                        </Card >
