...
import { getFramePos } from "../components/FrameItem";
...
const useDragAndDrop = (
  ...
  const insertFrame = (e, data) => {
    ...
    addCurrentStageData(newFrame);
  };

  const insertImage = (e, data) => {
    const isImageIcon = data?.src?.endsWith('svg');
    ...
    imageSrc.onload = () => {
      ...
      if (isImageIcon) {        
        fetch(`${data?.src}?${randomString()}` || "")
          .then(data => data.text())
          .then(data => {            
            const xmlString = data.replace(/\s+/g, ' ').trim();
            const findIdxSvg = xmlString.indexOf('<svg');
            const lastSvgString = `${xmlString.substring(findIdxSvg)}`;            
            ...
            const newImage = {
              ...
              attrs: {
                ...
                width,
                height,
                src: lastSvgString?.replaceAll('none', 'transparent'),
                zIndex: 0,
                brightness: 0,
                _filters: ["Brighten"],
                updatedAt: Date.now(),
    ...
    imageSrc.src = `${source}?${randomString()}`;
  ...
  const insertText = (e, data) => {
    ...
    addCurrentStageData(newText);
  };

  const insertShape = (e, data) => {
    ...
    addCurrentStageData(newShape);
  };

  const insertIcon = (e, data) => {
    ...
    addCurrentStageData(newIcon);
  };

  const insertLine = (e, data) => {
    const position = getFramePos(stageRef.current, e, 100, 100);
    const curvePoints
    ...
    addCurrentStageData(newLine);
  };

  const onDropOnStage = (dragSrc, e) => {
    if (!stageRef.current) {
      return;
    }
  ...
  const getItemsInThisFrame = (frame) => {
  ...
  const checkIsInFrame = (item) => {
    ...
    const frame = frameGroups.find((_item) => {
      const targetFrame = (_item).findOne("Rect");
      if (!targetFrame) {
        return false;
      }
  ...
  const moveToLayer = (item) => {
    const newParent = item.getLayer();
    item.getParent().children
      = (item.getParent().children)?.filter(
...
const isInFrame = (targetFrame, item) => {
