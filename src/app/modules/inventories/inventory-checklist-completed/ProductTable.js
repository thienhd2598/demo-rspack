import React, { memo, useMemo, useCallback, Fragment, useState, useEffect } from 'react';
...
const InventoryChecklistTable = ({ formikProps, warehouseId }) => {
        ...
        <>
            ...
            >
                                    ...
                                    return (
                                        ...
                                        >
                                            <a className={`nav-link font-weight-normal ${isActive ? 'active' : ''}`}
                                                style={{ fontSize: '13px' }}
                                                onClick={() => {
                                                    history.push(`/products/inventory/completed/${Number(paramsLink.id)}?${queryString.stringify({
                                                        ...params,
                                                        page: 1,
                                                        status: _tab.status
                                                    })}`)
                ...
                <table className="table table-borderless product-list table-vertical-center fixed">
                    <thead
                        style={{
                            position: 'sticky', top: 127, background: "#F3F6F9",
                            fontWeight: "bold",
                            fontSize: "14px",
                    ...
                    >
                        <tr className="font-size-lg">
                            ...
                            <th className='text-center' style={{ fontSize: '14px' }}>{formatMessage({ defaultMessage: 'Ghi chú' })}</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                {!error && (
                    <Pagination
                        ...
                        basePath={`/products/inventory/completed/${Number(paramsLink.id)}`}
                        emptyTitle=''
                    />
                )}
            </div>
            < div className='d-flex justify-content-end  mt-10' >
                <button className="btn btn-secondary mr-2" style={{ width: 150 }} onClick={e => {
                    e.preventDefault()
                    history.push('/products/inventory/list?status=complete')
