...
export default memo(() => {
    ...
    const [currentValue, setCurrentValue] = useState(
        diffHoursDay >= 0
            ? `${formatMessage({ defaultMessage: 'Tới' })} ${dayjs().startOf('hour').format('HH:mm')} ${formatMessage({ defaultMessage: 'hôm nay' })}`
            : dayjs().subtract(1, 'day').startOf('day').format('DD/MM/YYYY')
    );
    const [currentType, setCurrentType] = useState(
        params?.type_filter
        || (diffHoursDay >= 0 ? 'today' : 'hours')
    );
            ...
            return [
                {
                    ...
                    type: 'day',
                    value: <Calendar
                        value={params?.from ? dayjs(parseInt(params?.from) * 1000).toDate() : new Date()}
                        maxDate={new Date()}
                        locale={'vi'}
                        onChange={(value) => {
                            setShowFilterCalendar(false);
                ...
                },
                {
                    ...
                    type: 'week',
                    value: <Calendar
                        value={[params?.from ? dayjs(parseInt(params?.from) * 1000).toDate() : new Date(), params?.to ? dayjs(parseInt(params?.to) * 1000).toDate() : new Date()]}
                        maxDate={new Date()}
