...
const ReportTableProductSme = ({ dataTable, setSubTabTypeSme, status_tab }) => {
      ...
      <table className="table table-borderless product-list table-vertical-center fixed">
        ...
        >
          <tr>
            ...
            <th style={{ width: '70%' }}>{formatMessage({ defaultMessage: 'Sản phẩm' })}</th>
            <th className='text-center' style={{ width: '20%' }}>{STATUS_TAB['GMV'] == status_tab ? formatMessage({ defaultMessage: 'Doanh số hiệu quả' }) :
              <>
        ...
        <tbody>
          ...
          {!dataTable?.loading && dataTable?.data?.products?.length && dataTable?.data?.products?.map((item, index) => (
            <ReportProductRow status_tab={status_tab} item={item} key_row={index} />
          ))}
        </tbody>
      </table>
    </div>
...
export default ReportTableProductSme