...
import query_report_smeproductGMV from "../../../../../graphql/query_report_smeproductGMV";
...
const TableSmeProductGmv = ({ variables, dataStore }) => {
    ...
    const { data: dataSmeProductGMV, loading: loadingSmeProductGMV, } = useQuery(query_report_smeproductGMV, {
        variables: {
            ...variables,
    ...
    console.log(dataSmeProductGMV?.report_smeproductGMV_v2?.items?.length)
    useMemo(async () => {
        try {
            if (!dataSmeProductGMV?.report_smeproductGMV_v2?.items?.length) {
                setDataTable([])
                return []
            };

            setLoadingTable(true);
            const dataMappingSmeProduct = await queryGetSmeProductVariants(dataSmeProductGMV?.report_smeproductGMV_v2?.items?.map(product => product?.variantId));

            const parseData = dataSmeProductGMV?.report_smeproductGMV_v2?.items?.map(item => {
    ...
    }, [dataSmeProductGMV?.report_smeproductGMV_v2?.items]);
    ...
    const columns = [
        ...
        {
            title: <div>
                <span>{formatMessage({ defaultMessage: 'Doanh số hiệu quả' })}</span>
                <OverlayTrigger
                    overlay={
                        <Tooltip>
                            {formatMessage({ defaultMessage: 'Doanh số của sản phẩm đó ở tất cả các đơn hàng (không bao gồm đơn hoàn, huỷ) trong khoảng thời gian đã chọn, được thống kê dựa trên thời gian đặt hàng.' })}
                        </Tooltip>
                    }
                >
        ...
        },
        {
            title: <div>
                ...
                <OverlayTrigger
                    overlay={
                        <Tooltip>
                            {formatMessage({ defaultMessage: 'Doanh số của sản phẩm đó chia cho tổng doanh số ở tất cả các đơn hàng(không bao gồm đơn hoàn, huỷ) trong khoảng thời gian đã chọn, được thống kê dựa trên thời gian đặt hàng.' })}
                        </Tooltip>
                    }
                >
                    <span className="ml-2" style={{ position: 'relative', top: '-1px' }}>
        ...
        },
        {
            title: <div>
                ...
                <OverlayTrigger
                    overlay={
                        <Tooltip>
                            {formatMessage({ defaultMessage: 'Thay đổi về doanh số của các sản phẩm so với khoảng thời gian trước đó (tính theo %).' })}
                        </Tooltip>
                    }
                >
                    <span className="ml-2" style={{ position: 'relative', top: '-1px' }}>
    ...
    return (
        <Fragment>
            <div style={{ position: 'relative' }}>
                {(loadingTable || loadingSmeProductGMV) && (
                ...
                <Table
                    style={(loadingTable || loadingSmeProductGMV) ? { opacity: 0.4 } : {}}
                    className="upbase-table"
                    columns={columns}
                    data={(loadingTable || loadingSmeProductGMV || !dataSmeProductGMV?.report_smeproductGMV_v2?.items?.length) ? [] : dataTable}
                ...
                <PaginationModal
                    ...
                    emptyTitle=""
                    totalPage={dataSmeProductGMV?.report_smeproductGMV_v2?.totalPage}
                    // totalRecord={dataCrmProductByCustomer?.crmProductByCustomer?.paginatorInfo?.total || 0}
                    count={dataSmeProductGMV?.report_smeproductGMV_v2?.items?.length}
                />
            </div>
        </Fragment>
    )
...
export default memo(TableSmeProductGmv);