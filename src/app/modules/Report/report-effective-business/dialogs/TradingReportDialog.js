...
import { KEY_FINANCE_DASHBOARD_CHART } from "../../../Finance/trading-report/TradingReportHelper";
import query_cfGetAnalysisFinancePlatformChart from "../../../../../graphql/query_cfGetAnalysisFinancePlatformChart";
...
const TradingReportDialog = ({ currentKeyChart, channelsActive, onResetCurrentKeyChart, onToggleDrawer, variables }) => {
    ...
    const { data, loading } = useQuery(query_cfGetAnalysisFinancePlatformChart, {
        variables,
    ...
    const [revenueCostChart, isEmptyRevenueCostChart] = useMemo(
        () => {
            if (
                !data?.cfGetAnalysisFinancePlatformChart
                || data?.cfGetAnalysisFinancePlatformChart?.revenue_costs?.revenue_costs_item?.length == 0
                || currentKeyChart != 'profit'
            ) {
                return [[], true]
            }
            const dataRevenue = data?.cfGetAnalysisFinancePlatformChart?.revenue_costs?.revenue_costs_item;
    ...
    const [costAllocationRatio, costAllocationChart, isEmptyCostAllocationRatio] = useMemo(() => {
        if (
            !data?.cfGetAnalysisFinancePlatformChart
            || data?.cfGetAnalysisFinancePlatformChart?.revenue_costs?.cost_allocation?.length == 0
            || currentKeyChart != 'profit'
        ) return [[], [], true]

        const dataCostAllocation = flatten(data?.cfGetAnalysisFinancePlatformChart?.revenue_costs?.cost_allocation?.map(item => item?.items));
    ...
    const [ratioByChannelChart, isEmptyRatioByChannel] = useMemo(
        () => {
            if (!data?.cfGetAnalysisFinancePlatformChart || data?.cfGetAnalysisFinancePlatformChart?.[currentKeyChart]?.length == 0 || currentKeyChart == 'profit') {
                return [[], true]
            }
            const dataChart = data?.cfGetAnalysisFinancePlatformChart?.[currentKeyChart];
    ...
    const [allocattionByChannelRatio, allocattionByChannelChart, isEmptyAllocationByChannelChart] = useMemo(() => {
        if (
            !data?.cfGetAnalysisFinancePlatformChart
            || data?.cfGetAnalysisFinancePlatformChart?.[currentKeyChart]?.length == 0
            || currentKeyChart == 'profit'
        ) return [[], [], true]

        const dataAllocationChannel = flatten(data?.cfGetAnalysisFinancePlatformChart?.[currentKeyChart]?.map(item => item?.items?.map(i => ({ ...i, connector_channel_code: item?.connector_channel_code }))));
