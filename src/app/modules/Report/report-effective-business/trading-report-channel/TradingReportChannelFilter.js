...
import { ORDER_TYPE_OPTIONS, STATUS_ORDER } from '../../../Finance/trading-report/TradingReportHelper';
...
import mutate_cfExportAnalysisFinancePlatform from '../../../../../graphql/mutate_cfExportAnalysisFinancePlatform';
...
const TradingReportChannelFilter = ({ variables, currentChannel, channelsActive, loadingChannels }) => {
    ...
    const [cfExportAnalysisFinancePlatform, { loading: loadingCfExportFinanceReport }] = useMutation(mutate_cfExportAnalysisFinancePlatform);
    ...
    const onExportFinanceReport = useCallback(async () => {
        try {
            const { data } = await cfExportAnalysisFinancePlatform({
                variables
            });

            if (!!data?.cfExportAnalysisFinancePlatform?.success) {
                ...
                saveAs(data?.cfExportAnalysisFinancePlatform?.link, nameFileExport)
