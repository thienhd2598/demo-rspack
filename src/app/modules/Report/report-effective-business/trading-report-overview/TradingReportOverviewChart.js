...
import query_cfGetAnalysisFinanceChart from '../../../../../graphql/query_cfGetAnalysisFinanceChart';
...
const TradingReportOverviewChart = ({ variables }) => {
    ...
    const { data: dataChart, loading } = useQuery(query_cfGetAnalysisFinanceChart, {
        variables,
    ...
    const chartDashboard = useMemo(() => {
        const chart = {
            labels: dataChart?.cfGetAnalysisFinanceChart?.map(item => item?.label || '--'),
            datasets: Object.keys(omit(KEY_FINANCE_DASHBOARD_CHART, ['capital_price', 'operating_costs'])).map(key => {
                ...
                return {
                    ...
                    key,
                    data: dataChart?.cfGetAnalysisFinanceChart?.map(chart => ((key == 'revenue_sell' || key == 'profit') ? chart[key] : -Math.abs(chart[key])) || 0),
