...
export function ProductsFilter({ onDelete, onHide, setStoreDisconnect }) {
  ...
  const [scProductSyncUp] = useMutation(mutate_scProductSyncUp_raw, {
    ...
    <>
      ...
      >
        <div style={{
          ...
          onClick={e => {
            e.preventDefault()
            history.push(`/products/syncs?${queryString.stringify({
              ...params,
              page: 1,
              sync_status: null
            })}`)
        ...
        <div style={{ flex: 1, justifyContent: 'flex-end', display: 'flex' }} >
          <Dropdown drop='down'>
            ...
            <Dropdown.Menu>
              ...
              }} >{formatMessage({defaultMessage:'Đồng bộ'})}</Dropdown.Item>
              {/* <Dropdown.Item className="mb-1 d-flex" onClick={async e => {
                if (ids.length == 0) {
