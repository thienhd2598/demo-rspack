...
import query_sme_request_export_inventory_item_locations from "../../../../graphql/query_sme_request_export_inventory_item_locations";
const TableInventory = () => {
  ...
  const { data, error, loading, refetch } = useQuery(
    query_sme_request_export_inventory_item_locations,
    {
      variables: {
        limit,
        offset: (page - 1) * limit,
  ...
  useMemo(() => {
    ...
    const status = data?.sme_request_export_inventory_item_locations?.map(item => item.status) || []
    status?.includes(WAITING_STATUS) ? setTimePoll(1000) : setTimePoll(0)
  }, [data])

  let totalRecord = data?.sme_request_export_inventory_item_locations.length || 0;
      ...
      <table className="table table-borderless table-vertical-center fixed">
        ...
        >
          ...
          <RowTableInventory
            ...
            data={data?.sme_request_export_inventory_item_locations || []}
          />
        </tbody>
      </table>
      {!error && (
        <Pagination
          ...
          count={data?.sme_request_export_inventory_item_locations?.length}
