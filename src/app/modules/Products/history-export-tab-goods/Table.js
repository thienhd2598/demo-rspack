...
import query_warehouse_inventory_export_histories from "../../../../graphql/query_warehouse_inventory_export_histories";
...
const Table = ({ dataWarehouse }) => {
  ...
  const { data, error, loading, refetch } = useQuery(
    query_warehouse_inventory_export_histories,
    {
      variables: {
        limit,
        offset: (page - 1) * limit,
  ...
  useMemo(() => {
    ...
    const status = data?.warehouse_inventory_export_histories?.map(item => item.status) || []
    status?.includes(WAITING_STATUS) ? setTimePoll(1000) : setTimePoll(0)
  }, [data])

  let totalRecord = data?.warehouse_inventory_export_histories_aggregate?.aggregate?.count || 0;
      ...
      <table className="table table-borderless table-vertical-center fixed">
        ...
        >
          <tr className="font-size-lg">
            ...
            <th style={{ fontSize: "14px" }} width="140px">
              {formatMessage({ defaultMessage: "Thời gian thay đổi tồn" })}
            </th>
            {/* column3 */}
        ...
        >
          ...
          <RowTable
            ...
            data={data?.warehouse_inventory_export_histories || []}
          />
        </tbody>
      </table>
      {!error && (
        <Pagination
          ...
          count={data?.warehouse_inventory_export_histories?.length}
          basePath={"/products/history-export-tab-goods"}
