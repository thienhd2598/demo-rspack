...
import mutate_userUpdateProductUnitFromFile from '../../../../../graphql/mutate_userUpdateProductUnitFromFile';
import LoadingDialog from './LoadingDialog';

const ModalUpdateUnit = ({
    show,
    onHide,
}) => {
    ...
    const [userUpdateUnitFromFile, {loading: loadingUserUpdateUnit}] = useMutation(mutate_userUpdateProductUnitFromFile, {
    ...
    return (
        <>
            <LoadingDialog show={loadingUserUpdateUnit} />
            ...
            {show &&
                <Formik
                    ...
                    onSubmit={async values => {
                        let { data } = await userUpdateUnitFromFile({
                            variables: {
                                url: linkFile,
                            }
                        })
                        if (data?.userUpdateProductUnitFromFile?.success == 1) {
                            if (data?.userUpdateProductUnitFromFile?.total == data?.userUpdateProductUnitFromFile?.totalSuccess) {
                                setDataResults({
                                    errors: [],
                                    total: data?.userUpdateProductUnitFromFile?.total,
                                    totalSuccess: data?.userUpdateProductUnitFromFile?.totalSuccess
                                })
                                setModalResult(true)
                                resetData();
                                addToast(formatMessage({ defaultMessage: "Cập nhật đơn vị tính qua file thành công" }), { appearance: 'success' })
                            } else {
                                setDataResults({
                                    errors: data?.userUpdateProductUnitFromFile?.errors,
                                    total: data?.userUpdateProductUnitFromFile?.total,
                                    totalSuccess: data?.userUpdateProductUnitFromFile?.totalSuccess
                                })
                                setModalResult(true)
                            ...
                            addToast(data?.userUpdateProductUnitFromFile?.message || formatMessage({ defaultMessage: "Cập nhật đơn vị tính qua file không thành công" }), { appearance: 'error' });
                            resetData()
                        }
                    }}
                >
                            ...
                            >
                                <>
                                    <Modal.Header closeButton={true}>
                                        <Modal.Title>
                                           {formatMessage({ defaultMessage: 'Cập nhật đơn vị tính theo File' })}
                                        </Modal.Title>
                                    </Modal.Header>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        if (process.env.REACT_APP_MODE !== 'PROD') {
                                                            saveAs(
                                                                'https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Stagging/Cap_nhat_DVT.xlsx',
                                                                'Cap_nhat_DVT'
                                                            )
                                                        } else {
                                                            saveAs(
                                                                'https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/prod/Cap_nhat_DVT.xlsx',
                                                                'Cap_nhat_DVT'
                                                            )
                                                        }
                                                    }}
...
export default memo(ModalUpdateUnit);