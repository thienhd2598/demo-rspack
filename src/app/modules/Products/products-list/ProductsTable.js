...
import sme_catalog_product from '../../../../graphql/query_sme_catalog_product'
...
import query_sme_catalog_product_v2 from "../../../../graphql/query_sme_catalog_product_v2";
...
export function ProductsTable({ onDelete, onHide, onShow, onCreateMutilTag, nameSearch }) {
  ...
  let type = useMemo(() => {
    ...
    return uiHelpers.getQueryByType(params.type)
  }, [params.type]);
  ...
  let whereCondition = useMemo(
    ...
    }, [nameSearch, type, has_origin_image, is_combo, tags, scProductMapping]
  );

  const { data, loading, error, refetch } = useQuery(query_sme_catalog_product_v2, {
    variables: {
      limit,
      offset: (page - 1) * limit,
          ...
          <tr className="font-size-lg">
            <th style={{ fontSize: '14px' }}>
              <Checkbox
                ...
                //     setIds([])
                //   } else {
                //     setIds((data?.sme_catalog_product || []).map(_product => {
                //       return _product
                //     }).filter(_pro => !!_pro))
                //   }
                // }}
            ...
            <th style={{ fontSize: '14px' }} width="11%">
