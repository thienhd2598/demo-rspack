...
function ProductImageEditDialog({ show, onHide, dataCrop }) {
  ...
  const enableAccept = !!detail && (width >= 500 && width <= (dataCrop?.maxSize || 1024) && height >= 500 && height <= (dataCrop?.maxSize || 1024))
  return (
    <Modal
      show={show}
      onHide={onHide}
    ...
    >
          ...
          <div className='col-4 my-4' >
            ...
            <div className='row mb-4' >
              <div className='col-12' >
                ...
                >{formatMessage({defaultMessage:'Cắt tự do'})}</button>
                <button className={`btn  mr-2  ${ratio == 1 ? 'btn-primary active' : 'btn-secondary'}`} style={{ width: 100 }}
                  onClick={e => {
                    setRatio(1)
            ...
            <div className='row mb-4 mr-0' >
              <div className='col-6' >
                ...
                <NumberFormat
                  ...
                  max={dataCrop?.maxSize || 1024}
                  min={0}
                  suffix=' px'
                  isAllowed={({ floatValue }) => !floatValue || floatValue <= (dataCrop?.maxSize || 1024)}
                  allowNegative={false}
            ...
            <div className='row mb-4' >
              <div className='col-12' >
                ...
                >{loading ? <span className="spinner m-4"  ></span> : "Chuẩn hoá ảnh"}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <Modal.Footer className="form form-group py-2">
        ...
        <span className='font-size-xs text-info' ><em>* {formatMessage({defaultMessage:'Kích thước: Tối thiểu 500x500, tối đa'})} {(dataCrop?.maxSize || 1024)}x{(dataCrop?.maxSize || 1024)}</em></span>
        <button
          type="button"
          onClick={onHide}
