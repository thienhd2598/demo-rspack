...
import {OPTIONS_STATUS_PRODUCT} from './contants'
...
const RowTableInventory = ({ refetch, data, dataWarehouse }) => {
  ...
  const productStatus = (status) => {
    if (status == "instock") {
      return formatMessage({defaultMessage:"Còn hàng"});
    }
    if (status == "outstock") {
      return formatMessage({defaultMessage:"Hết hàng"});
    }
    if (status == "near_outstock") {
      return formatMessage({defaultMessage:"Sắp hết hàng"});
    }
    if (status == "preallocate") {
      return formatMessage({defaultMessage:"Tạm ứng"});
    }
  };
  ...
  const [inventoryCreateExportRetry, { loading: loadingRetry }] = useMutation(
    mutate_InventoryCreateExportRequestOutput
  );
  return (
    <>
