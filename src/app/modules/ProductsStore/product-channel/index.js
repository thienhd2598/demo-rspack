                    ...
                    channels[connector_channel_code].logistics?.map(_logistic => {
                      return (
                        <div key={`_logistic--${_logistic.ref_channel_id}`} className="row">
                          <label className="col-6 col-form-label">{_logistic.channel_name} ({formatMessage({ defaultMessage: 'Tối đa {value}g' }, { value: formatNumberToCurrency(_logistic.max_weight) })})</label>
                          {
