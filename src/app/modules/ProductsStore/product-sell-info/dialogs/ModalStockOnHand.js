...
const ModalStockOnHand = ({ show, onHide }) => {
    ...
    const initialValues = useMemo(() => {
        const init = scWarehouses?.reduce(
            (result, wh) => {
                result[`${wh?.value}-stockOnHand`] = valuesProduct[`${wh?.value}-stockOnHand`] || 0;
                return result;
            }, {}
        );

        return init;
    }, [scWarehouses, valuesProduct]);

    const validateSchema = useMemo(() => {
        return Yup.object().shape(scWarehouses.reduce(
            (result, wh) => {
                result[`${wh?.value}-stockOnHand`] = Yup.number()
                            ...
                            <Modal.Body className="overlay overlay-block cursor-default" style={{ padding: 0 }}>
                                ...
                                <div style={{ padding: '0rem 1rem' }}>
                                    <div className="d-flex align-items-center mt-4 mb-2">
                                        <span style={{ width: '50%' }}>SKU: {valuesProduct[`origin_sku`] || '--'}</span>
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (isErrorForm) {
                                                ...
                                                scWarehouses.forEach(wh => {
                                                    setFieldValue(
                                                        `${wh?.value}-stockOnHand`,
                                                        values[`${wh?.value}-stockOnHand`] || 0
                                                    );
                                                })
                                                onHide();
                                            }
