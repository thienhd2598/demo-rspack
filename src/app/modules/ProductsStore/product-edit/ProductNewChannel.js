    ...
    if (!!data?.scGetSmeProduct && channelLoaded != null) {
      ...
      (data?.scGetSmeProduct || []).forEach(_product => {
        ...
        (_product.attribute_values || []).forEach(_attribute => {
          ...
          if (_attribute.input_type == ATTRIBUTE_VALUE_TYPE.DATE || _attribute.input_type == ATTRIBUTE_VALUE_TYPE.DATE_MONTH || _attribute.input_type == ATTRIBUTE_VALUE_TYPE.TIMESTAMP)
            setFieldValue(`${_product.connector_channel_code}-property-${_attribute.id}`, _valueAttribute)
          if (_attribute.input_type == ATTRIBUTE_VALUE_TYPE.NUMERIC || _attribute.input_type == ATTRIBUTE_VALUE_TYPE.NUMERIC_FLOAT || _attribute.input_type == ATTRIBUTE_VALUE_TYPE.NUMERIC_INT)
            setFieldValue(`${_product.connector_channel_code}-property-${_attribute.id}`, _valueAttribute)
      ...
      <div className="col-9">
            ...
            }
            {
              channelKeys.length == 0 && <Card >
                <CardBody style={{ textAlign: 'center' }} >
                  <p className='mb-1' >
                    {formatMessage({ defaultMessage: 'Bạn chưa kết nối với gian hàng/kênh bán nào' })}.
                  </p>
                  <p style={{ fontStyle: 'italic' }}>
                    {formatMessage({ defaultMessage: 'Vui lòng kết nối để thực hiện Đăng bán sản phẩm.Lưu ý: Thông tin của bạn đã được lưu nháp trên Sản phẩm Upbase' })}.
                  </p>
                  <Link className="btn btn-primary" style={{ width: 150 }} to='/setting/channels'>
                    {formatMessage({ defaultMessage: 'KẾT NỐI NGAY' })}
                  </Link>
                </CardBody>
              </Card>
            }
        ...
        >
          <Modal.Body className="overlay overlay-block cursor-default text-center">
            <div className="mb-4">
              {formatMessage({ defaultMessage: 'Đã cập nhật xong thông tin, sản phẩm đang được đồng bộ lên sàn' })}
            </div>
            ...
            <div className="form-group mb-0">
              ...
              >
                <span className="font-weight-boldest">
                  {formatMessage({ defaultMessage: 'Về sản phẩm UpBase' })}
                </span>
              </Link>
              <Link
                type="button"
                to='/product-stores/syncs'
                className={`btn btn-primary font-weight-bold`}
                style={{ width: 180 }}
              >
                <span className="font-weight-boldest">
                  {formatMessage({ defaultMessage: 'Xem thông tin đồng bộ' })}
                </span>
              </Link>
            </div>
          </Modal.Body>
