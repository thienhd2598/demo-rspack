import React, { memo, useState, useMemo, useCallback, Fragment, useEffect, useLayoutEffect } from "react";
...
const ProductUpdateTagImage = () => {
    const { appendBreadcrumbs, setTitle } = useSubheader();
    const { formatMessage } = useIntl();
    setTitle(formatMessage({ defaultMessage: 'Danh sách sản phẩm sàn' }));
    ...
    const onUpdate = useCallback(
        async (e) => {
            ...
            if (res?.data?.scUpdateMultiProductOriginImageTag?.success) {
                history.push(`/product-stores/list?channel=${products?.[0]?.connector_channel_code}`);
