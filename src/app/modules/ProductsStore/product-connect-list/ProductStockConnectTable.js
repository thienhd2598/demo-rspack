...
import ProductStockConnectRow from "./ProductStockConnectRow";
...
const ProductStockConnectTable = () => {
    ...
    const STATUS_PRODUCT_CONNECT = [
        ...
        { title: formatMessage({ defaultMessage: 'Chưa liên kết' }), value: 0 },
        { title: formatMessage({ defaultMessage: 'Đã liên kết' }), value: 1 },
    ];
    ...
    const status = useMemo(() => {
        ...
        return {
            product_status: Number(params?.status)
        }
    }, [params?.status]);
        ...
        <>
            ...
            <div>
                <table className="table product-list table-borderless product-list table-vertical-center fixed"  >
                    <thead style={{ position: 'sticky', top: 90, background: '#F3F6F9', fontWeight: '500', fontSize: '14px', borderRight: '1px solid #d9d9d9', borderLeft: '1px solid #d9d9d9' }}>
                        <tr>
                            <th width="35%" style={{ fontSize: '14px', fontWeight: 'bold' }}>
                                {formatMessage({ defaultMessage: 'Sản phẩm hàng hóa' })}
                            </th>
                            <th width="20%" style={{ fontSize: '14px', fontWeight: 'bold' }}>
                                {formatMessage({ defaultMessage: 'Hàng hóa' })}
                            </th>
                            <th width='35%' style={{ fontSize: '14px', fontWeight: 'bold' }}>
                                {formatMessage({ defaultMessage: 'Sản phẩm hàng hóa kho' })}
                            </th>
                    ...
                    <tbody>
                        ...
                        {
                            !error && !loading && data?.scGetProductVariants?.variants?.map(_variant => {
                                return <ProductStockConnectRow
                                    key={`product-row-${_variant.id}`}
                                    variant={_variant}
                                    onLink={(id, sme_product_variant_id) => setScId({ id, sme_product_variant_id })}
                                    onRemoveLink={params => {
                                    ...
                                    onLinkVariant={(id, sme_product_variant_id) => setScVariantId({ id, sme_product_variant_id })}
            ...
            <ProductConnectVariantDialog
                ...
                scVariantId={scVariantId?.id}
                sme_product_variant_id={scVariantId?.sme_product_variant_id}
            />
            <ModalCombo
                dataCombo={dataCombo}
...
export default memo(ProductStockConnectTable);