...
import query_getFinanceDashboardTable from '../../../../../graphql/query_getFinanceDashboardTable';
...
const TradingReportOverviewTable = ({ variables }) => {
    ...
    const { data, loading } = useQuery(query_getFinanceDashboardTable, {
        variables,
    ...
    const dataTradingReportOverview = useMemo(
        () => {
            if (!data?.getFinanceDashboardTable) return [];
            ...
            const {
                ...
                , profit_margin_growth, revenue_sell_growth,
                capital_price_gift, capital_price_sell, capital_price_gift_growth,
                capital_price_sell_growth, compare_capital_price_gift,
                compare_capital_price_sell,
                money_about_wallet, money_about_wallet_growth,
                compare_money_about_wallet
            } = data?.getFinanceDashboardTable || {};

            const dataTable = [
                {
                    key: 'profit',
                    code: '',
                    title: formatMessage({ defaultMessage: 'Lợi nhuận (05 - 06 - 08 - 09)' }),
                ...
                },
                {
                    key: 'net_revenue',
                    code: '03',
                    title: formatMessage({ defaultMessage: 'Doanh thu thuần (03 = 01 - 02)' }),
                    value: net_revenue ?? "--",
                ...
                },
                {
                    ...
                    value: capital_price ?? "--",
                    compareValue: compare_capital_price ?? "--",
                    type: capital_price_growth >= 0 ? 'increase' : 'decrease',
                    ratio: capital_price_growth,
                    children: [
                        {
                            key: 'Giá vốn hàng bán',
                            code: '4.1',
                            title: 'Giá vốn hàng bán',
                            value: capital_price_sell ?? "--",
                            compareValue: compare_capital_price_sell ?? "--",
                            type: capital_price_sell_growth >= 0 ? 'increase' : 'decrease',
                            ratio: capital_price_sell_growth,
                        },
                        {
                            key: 'Giá vốn quà tặng',
                            code: '4.2',
                            title: 'Giá vốn quà tặng',
                            value: capital_price_gift ?? "--",
                            compareValue: compare_capital_price_gift ?? "--",
                            type: capital_price_gift_growth >= 0 ? 'increase' : 'decrease',
                            ratio: capital_price_gift_growth,
                        }
                    ]
                },
                {
                    key: 'gross_profit',
                    code: '05',
                    title: formatMessage({ defaultMessage: 'Lợi nhuận gộp (05 = 03 - 04)' }),
                    value: gross_profit ?? "--",
                    compareValue: compare_gross_profit ?? "--",
                    type: gross_profit_growth >= 0 ? 'increase' : 'decrease',
                    ratio: gross_profit_growth
                },
                {
                    key: 'fees_platform',
                    code: '06',
                ...
                },
                {
                    key: 'operating_costs',
                    code: '09',
                    title: formatMessage({ defaultMessage: 'Chi phí vận hành' }),
                    value: operating_costs?.value ?? "--",
                    compareValue: operating_costs?.compare_value ?? '--',
                    type: operating_costs?.growth >= 0 ? 'increase' : 'decrease',
                    ratio: operating_costs?.growth,
                    children: operating_costs?.items?.map((item, index) => ({
                        key: item?.label,
                        code: `9.${index + 1}`,
                        title: item?.label,
                        value: item?.cost ?? '--',
                        ...
                        <Tooltip>
                            <div className='d-flex flex-column align-items-center'>
                                <span className='mb-2 fs-14'>
                                    ({data?.getFinanceDashboardTable?.previous_label})
                                </span>
