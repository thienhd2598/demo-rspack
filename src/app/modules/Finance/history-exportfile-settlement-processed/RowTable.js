...
const RowTable = ({ channels, item }) => {
  ...
  const [timeFrom, timeTo, listStore] = useMemo(() => { 
    console.log(JSON.parse(item?.params_payload))
  ...
  const DIFFERENCE_CODE = {
    ALL: 0,
    ABNORMAL: 2,
    BALANCE: 1,
  }

  const status_diffeerence = {
    [DIFFERENCE_CODE.ALL]: formatMessage({ defaultMessage: "Tất cả" }),
    [DIFFERENCE_CODE.ABNORMAL]: formatMessage({ defaultMessage: "Có" }),
    [DIFFERENCE_CODE.BALANCE]: formatMessage({ defaultMessage: "Không" }),
  }
  ...
  const [cfRetryExportSettlement, { loading: retryLoading }] = useMutation(
    ...
    onCompleted: (data) => {
      ...
      addToast(data?.cfRetryExportSettlement.message || 'Có lỗi xảy ra', {
        appearance: "error",
      });
    }
  });
  return (
    <tr>
      ...
      <td className='text-center'>{status_diffeerence[item?.settlement_abnormal]}</td>
