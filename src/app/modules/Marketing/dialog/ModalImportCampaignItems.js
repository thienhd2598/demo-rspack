...
import mutate_mktValidateExcelSku from '../../../../graphql/mutate_mktValidateExcelSku';

const ModalImportCampaignItems = ({
    ...
    const [mktValidateExcelSku, { loading: loadingMktImportCampaign }] = useMutation(mutate_mktValidateExcelSku);
    ...
    const onImportCustomer = useCallback(async () => {
        let { data } = await mktValidateExcelSku({
            variables: {
                file_url: linkFile,
                store_id: storeId,
        ...
        if (data?.mktValidateExcelSku?.success) {
            resetData();
            setDataResultImport(data?.mktValidateExcelSku);
        } else {
            addToast(data?.mktValidateExcelSku?.message || formatMessage({ defaultMessage: "Nhập file dữ liệu không thành công" }), { appearance: 'error' });
        }
    }, [linkFile]);
        ...
        <Fragment>
            ...
            >
                <Modal.Header closeButton={true}>
                    <Modal.Title>
                        {formatMessage({ defaultMessage: 'Nhập file danh sách {target} sàn' }, { target: targetHandled })}
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body className="overlay overlay-block cursor-default px-10 pb-15" style={{ position: 'relative' }}>
                    ...
                    <div className='row'>
                        <div className='col-3'>{formatMessage({ defaultMessage: 'Tải exel mẫu' })}</div>
                        <div className='col-9 px-0 mb-6'>
                            <button
                                type="button"
                                onClick={() => {                                    
                                    saveAs('https://prod-statics.s3.ap-southeast-1.amazonaws.com/template/Filemaudanhsachsanpham_CTKM.xlsx');
                                }}
                                className="btn"
...
export default memo(ModalImportCampaignItems);
