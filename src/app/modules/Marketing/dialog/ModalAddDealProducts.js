...
const ModalAddDealProducts = ({
    ...
    const { logisticsChannel } = useDealContext();
    ...
    const { data, loading } = useQuery(query_scGetProductsModal, {
        variables: {
            ...
            ...(search?.shippingUnits?.length > 0 ? {
                shipping_units: search?.shippingUnits?.map(item => item?.value)
            } : {}),
            ...(!!search.searchText ? {
                q: search.searchText,
            } : {}),
            ...(filterOutIds?.length > 0 ? {
                filter_out_ids: filterOutIds
            } : {})
        },
        fetchPolicy: 'cache-and-network'
    });
    ...
    const { data: dataProductGVM, loading: loadingProductGVM } = useQuery(query_report_productSoldScByGMV, {
        ...
        onCompleted: async (data) => {
            ...
            console.log(`CHECK DATA: `, scProductVariants?.map(item => item?.product), filterOutIds);
            setDataTableGVM([...new Set(scProductVariants?.map(item => item?.product)?.filter(product => !filterOutIds?.some(id => id == product?.id)))]);
        }
    });
        ...
        },
        {
            ...
            render: (_item, record) => {
                const logistics = logisticsChannel
                    ?.filter(lg => record?.ref_logistic_channel_id?.some(id => id == lg?.value))
                    ?.map(lg => lg?.label);
                const isExpand = expands?.some(id => id == record?.id);
                ...
                return <div className="d-flex justify-content-center align-items-center">
                    <span>{logistics?.slice(0, isExpand ? logistics?.length : 2)?.join(', ')} {logistics?.length > 2 && <span className="text-info ml-1 cursor-pointer" onClick={() => {
                        setExpands(prev => isExpand ? prev?.filter(id => id != record?.id) : prev.concat(record?.id))
                    }}>
                ...
                <Modal.Body className="overlay overlay-block cursor-default pb-0">
                    ...
                    {currentTab == 'list' && <div className='row mb-4'>
                        ...
                        <div className="col-4">
                            <Select
                                className="w-100"
                                placeholder={formatMessage({ defaultMessage: "Chọn vận chuyển" })}
                                isMulti
                                isClearable
                                value={search?.shippingUnits}
                                styles={{
                                    container: (styles) => ({
                                        ...styles,
                                        zIndex: 9
                                ...
                                onChange={values => {
                                    const paramsShippingUnit = values?.length > 0 ? values : [];

                                    setSearch({ ...search, shippingUnits: paramsShippingUnit, page: 1 })
                                }}
                                options={logisticsChannel}
                            />
                        </div>
                    </div>}
...
export default memo(ModalAddDealProducts);