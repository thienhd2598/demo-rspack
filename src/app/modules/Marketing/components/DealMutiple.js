    ...
    const columns = [
        {
            ...
            render: (item, record, index) => {
                ...
                return <div className="d-flex flex-column">
                    <span className="cursor-pointer mb-1" onClick={() => {
                        if (coming_soon || happening || record?.status == 1) {
                            if (record?.status != 1) {
                                window.open(`/marketing/voucher/${record?.id}?action=edit&type=2`, "_blank")
                            } else {
                                window.open(`/marketing/voucher/${record?.id}?action=edit`, "_blank")
                            }
                        }

                        if (finished) {
                            window.open(`/marketing/voucher/${record?.id}?type=2`, "_blank")
                        }
                    }}>
                        {record?.name}
                    </span>
                    {!!record?.code && <span className="text-muted fs-12">{formatMessage({ defaultMessage: 'Mã voucher: {name}' }, { name: record?.code })}</span>}
                </div>
            }
        },
        {
        ...
        },
        {
            ...
            width: '25%',
            render: (item, record, index) => {
                // const isSyncErrorCampaignItem = !!record?.campaign?.filter(item => item?.sync_error_message).length
                const isSyncErrorCampaign = record?.sync_error_message

                return <Fragment>
                    {!!(record?.sync_status == 2 && !record.sync_error_message) &&
                        <div>
                            <p style={{ color: '#3DA153' }}>Đã đồng bộ</p>
                            {record?.synced_at && <p >{dayjs.unix(record?.synced_at).format('DD/MM/YYYY HH:mm')} </p>}
                        </div>}
                    {!!(record.sync_status == 1 && !record?.sync_error_message) && <span >Chưa đồng bộ</span>}
                    {(isSyncErrorCampaign) && <div className="d-flex justify-content-center align-items-center">
                        <span className="mr-2" style={{ color: '#F80D0D' }}>Đồng bộ lỗi</span>
                        <OverlayTrigger
                            overlay={
                                <Tooltip>
                        ...
                        >
                            <span role='button' className="mr-2">
...
export default memo(DealMutiple);