...
import InfoCampaignTemplate from './InfoCampaignTemplate';
import DetailCampaignTemplate from './DetailCampaignTemplate';
...
const CampaignTemplateCreate = memo(({ }) => {
    ...
    return (
        <Fragment>
            ...
            />}
            <LoadingDialog show={loadingRemoveFrameImage || loadingMktSaveCampaignTemplate} />
                                    ...
                                    onClick={async (e) => {
                                        ...
                                        if (Object.keys(totalError)?.filter((item) => {
                                            ...
                                            const { data } = await mktSaveCampaignTemplate({
                                                variables: {
                                                    campaign_template_info: {
                                                        ...
                                                        campaign_type: paramsQuery?.typeCampaign == 'discount' ? 1 : 2
                                                    },
                                            ...
                                            if (data?.mktSaveCampaignTemplate?.success) {
                                                addToast(formatMessage({ defaultMessage: 'Tạo chương trình khuyến mại hàng loạt thành công' }), { appearance: 'success' })
                                                history.push(
                                                    `/marketing/sale-list?typeCampaign=template`
                                                )
                                            } else {
                                                addToast(data?.mktSaveCampaignTemplate?.message || formatMessage({ defaultMessage: 'Tạo chương trình khuyến mại hàng loạt thất bại' }), { appearance: "error" })
                                            }
                                        }
                                    }}
                                >
...
const CamPaignCreateTemplateWrapper = () => {
    return (
        <MarketingProvider>
...
export default CamPaignCreateTemplateWrapper;
