...
const CampaignCreate = memo(({ }) => {
  ...
  const { data: saleDetail, loading: loadingDetail, refetch, error } = useQuery(query_mktFindCampaign, {
    variables: { id: +paramsQuery?.id },
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { title: typeCampaign == 'discount' ? 'Chiết khấu sản phẩm' : 'FlashSale' },
    ]);
  }, [typeCampaign]);

  const { loading, data: dataFrameByPk } = useQuery(query_sme_catalog_photo_frames_by_pk, {
    variables: {
      id: saleDetail?.mktFindCampaign?.campaignScheduleFrame?.frame_id
    },
    skip: !saleDetail?.mktFindCampaign?.campaignScheduleFrame?.frame_id
  });
  ...
  useMemo(() => {
    ...
    buildStateFromCampaignItems({
      type: saleDetail?.mktFindCampaign?.item_type == 1 ? 'product' : 'variant',
      infoCampaign: {
        frameInfo,
        ...saleDetail?.mktFindCampaign
      }
    })
  }, [saleDetail, dataFrameByPk])
  ...
  return (
    <Fragment>
      <Helmet
        titleTemplate={'UB - ' + `${typeCampaign == 'discount' ? 'Chiết khấu sản phẩm' : 'FlashSale'}`}
        defaultTitle={'UB - ' + `${typeCampaign == 'discount' ? 'Chiết khấu sản phẩm' : 'FlashSale'}`}
      >
        <meta
          name="description"
          content={'UB - ' + `${typeCampaign == 'discount' ? 'Chiết khấu sản phẩm' : 'FlashSale'}`}
        />
      </Helmet>
      ...
      />}
      <LoadingDialog show={loadingRemoveFrameImage || loadingCreateCampaign} />
              ...
              <div className='form-group d-flex justify-content-end mt-8 group-button-fixed-bottom pr-10' style={{ zIndex: 9 }}>
                ...
                <button
                  ...
                  onClick={async (e) => {
                    ...
                    console.log(`E NGUYET CHECK: `, totalError);
                    ...
                    if (Object.keys(totalError)?.filter((item) => {
                      ...
                      const { data } = await createCampaign({
                        variables: {
                          campaign_info: {
                            connector_channel_code: paramsQuery.channel,
                            ...
                            type: paramsQuery?.typeCampaign == 'discount' ? 1 : 2
                          },
                      ...
                      if (data?.mktSaveCampaign?.success) {
                        addToast(formatMessage({ defaultMessage: 'Tạo chương trình khuyến mại thành công' }), { appearance: 'success' })
                        history.push(
                          `/marketing/sale-list`
                        )
                      } else {
                        addToast(data?.mktSaveCampaign?.message, { appearance: "error" })
                        history.push(
                          `/marketing/sale-list`
                        )
                      }
...
export default CamPaignCreateWrapper;
