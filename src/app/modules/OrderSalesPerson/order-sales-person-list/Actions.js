...
const Actions = ({onShowImportDialog, onShowExportDialog, onSetDataSmeNote, onOpenConfirmDialog, dataSelectedOrder }) => {
  const location = useLocation();
    const { formatMessage } = useIntl()
    const history = useHistory();
    const params = queryString.parse(location.search.slice(1, 100000));
  return (
    <div style={{ position: "sticky", top: 45, background: "#fff", zIndex: 55 }}>
    ...
    {params?.type == 'pending' && !params?.is_old_order && (
         ...
         <div className="mr-4 text-primary" style={{ fontSize: 14 }}>{formatMessage({ defaultMessage: "Đã chọn:" })} {dataSelectedOrder?.length}</div>
        ...
        {!params?.is_old_order && (
          <AuthorizationWrapper keys={['order_sales_person_create_manual']}>
            <Dropdown drop='down'>
              ...
              <Dropdown.Menu>
                  <Dropdown.Item className="mb-1 d-flex" onClick={(e) => history.push({
                    pathname: '/order-sales-person/create-manual',
                    state: { urlRedirect: '/order-sales-person/list-order' }
                })}>
                    {formatMessage({ defaultMessage: "Tạo đơn" })}
                  </Dropdown.Item>
                  <Dropdown.Item className="mb-1 d-flex" onClick={(e) => {
        ...
        <AuthorizationWrapper keys={['order_sales_person_export_file']}>
          ...
          <div>
            <button className="btn btn-secondary btn-elevate"
              onClick={(e) => history.push('/order-sales-person/history-export-file-sales-person-order')}
            >
              <HistoryRounded />
            </button>
          </div>
