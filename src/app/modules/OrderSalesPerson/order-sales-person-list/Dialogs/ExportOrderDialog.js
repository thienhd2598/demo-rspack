...
import { STATUS_PACK_MAIN_ORDER_TAB, STATUS_PACK_TAB } from "../../../Order/OrderUIHelpers";
...
function ExportOrderDialog({ show, onHide }) {
  ...
  const [_statusParse] = useMemo(() => {
    ...
    let _status = (status || STATUS_PACK_TAB.slice(1, 8)).map(_sts => {
  ...
  }, [store, shipping,_statusParse, user,valueRangeTime, channel, params?.is_old_order])
            ...
            onClick={async e => {
              ...
              try {
                ...
                if (data?.scExportOrder?.success == 1) {
                  ...
                  onHide()
                  history.push(`/order-sales-person/history-export-file-sales-person-order`)
                } else {
