...
export default memo(({ params, order, op_connector_channels }) => {
    ...
    const [time_from , time_to, stores, template] = useMemo(() => {
            const {time_from , time_to, list_store, template_type} = order?.params_payload ? JSON.parse(order?.params_payload) : {}
    ...
    return (
        <Fragment>
            <tr>
                ...
                <td >
                    {!!link_file_export ? <span className="btn btn-primary cursor-pointer"
                     onClick={e => {
                        e.preventDefault()
                        e.stopPropagation()
