...
const OrderSalesPersonFilterDrawer = ({ user, isOpenDrawer, onToggleDrawer, whereCondition }) => {
    ...
    const { data } = useQuery(query_sme_sub_users, {
        fetchPolicy: 'cache-and-network',
        onCompleted: (data) => {
            setAllSubUser(data?.sme_sub_users?.map(user => ({
                label: user?.username,
                value: user?.username
            })))
        }
    ...
    const { data: coGetShippingCarrier, refetch: refetchCoGetShippingCarrier } = useQuery(query_coGetShippingCarrierFromListPackage, {
        variables: {
            search: {
                ...
                is_connected: 1,
                list_status: ['ready_to_ship', 'pending']
            }
        },
        fetchPolicy: 'cache-and-network'
    });
    ...
    }, [params?.shipping_unit, params?.payments, isOpenDrawer, params?.type_parcel, params?.users]);
    ...
    const [currentPayments, optionsPayments] = useMemo(() => {
        ...
        let currentPayments = optionsPayments?.filter(_option => parseParamsPayments?.some(param => param == _option?.value));
        return [currentPayments, optionsPayments]
    }, [coGetPaymentMethod, params, filterParams])

    const [currentUsers, optionsUser] = useMemo(() => {
        let parseParamsUsers = filterParams?.users?.split(',')
        const options = user?.is_subuser ? [...allSubUser] : [{label: user?.email, value: user?.email},...allSubUser]
        let currentUsers = options?.filter(_option => parseParamsUsers?.some(param => param == _option?.value));
        return [currentUsers, options]
    }, [params, filterParams, user, allSubUser])
    ...
    const onConfirmFilterProducts = useCallback(() => {
            let filtered = {
                ..._.omit(params, ['shipping_unit', 'payments', 'type_parcel', 'users', 'after_sale_type']),
                ...filterParams,
                page: 1
            };
        ...
        <div className="drawer-filter-wrapper d-flex flex-column justify-content-between">
            <div className="d-flex flex-column">
                ...
                <div style={{ overflow: 'scroll', overflowX: 'hidden', height: '84vh' }}>
                    ...
                    <div className="drawer-filter-item p-4">
                        <p className="mb-2 font-weight-bold">{formatMessage({ defaultMessage: "Người phụ trách" })}</p>
                        <Select
                            placeholder={formatMessage({ defaultMessage: "Tất cả" })}
                            isMulti
                            isClearable
                            value={currentUsers || []}
                            onChange={values => {
                                let paramsUsers = values?.length > 0
                                    ? _.map(values, 'value')?.join(',')
                                    : undefined;

                                onUpdateFilterParams('users', paramsUsers);
                            }}
                            options={optionsUser}
                        />
                    </div>
