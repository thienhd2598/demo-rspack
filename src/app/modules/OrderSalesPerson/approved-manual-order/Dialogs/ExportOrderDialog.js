...
import { OPTIONS_TEMPLATE,STATUS_PACKAGES,STATUS_SALES_ORDER_FILTER} from "../Constant";
import queryString from 'querystring';
import { TooltipWrapper } from '../../../Finance/payment-reconciliation/common/TooltipWrapper'

function ExportOrderDialog({ show, onHide }) {
  ...
  useMemo(() => {
    if (!!params?.is_old_order) {
      setValueRangeTime([new Date(dayjs().subtract(96, "day").startOf("day")), new Date(dayjs().subtract(90, "day").startOf("day")),]);
      return;
    } else {
  ...
  const [_statusParse] = useMemo(() => {
    let _status = (!status ? STATUS_SALES_ORDER_FILTER.slice(1, 100)?.filter(item => item?.status) : status).map(_sts => {
      if(_sts?.status == 'ready_to_ship') {
  ...
  }, [store, shipping,_statusParse, valueRangeTime, channel, params?.is_old_order])

  const { data: dataOrderAggregate, loading: loadingOrderAggregate } = useQuery(query_scExportOrderAggregate, {variables: {..._whereCondition}});

  const { data: dataShippingCarrier } = useQuery(query_coGetShippingCarrierFromListPackage, {
    variables: {
        search: {
            list_source: ['manual'],
            ...
            ...(!!_statusParse.length > 0 ? { list_status: _statusParse } : {}),
            is_connected: 1,
            ...(params?.is_old_order ? {is_old_package: 1} : {})
        }
    },
    fetchPolicy: 'cache-and-network'
});
...
const shippingUnitOptions = useMemo(() => {
  ...
  const [mutate, { loading }] = useMutation(mutate_scExportOrder)
      ...
      <Modal.Body className="overlay overlay-block cursor-default" >
        ...
        <div className="col-12" >
          ...
          <div className="row mt-3 display-flex align-items-center " >
            ...
            <div className="col-8" >
              <Select options={STATUS_SALES_ORDER_FILTER.slice(1, 100).map((_status, _idx) => {
                return {
                  ..._status,
                  label: _status.title,
                  value: _idx
                ...
                styles={{container: (styles) => ({...styles,zIndex: 97}),}}
              />
            </div>
          </div>
            <div className="row mt-3 display-flex align-items-center " >
          ...
          <div className="row mt-3 display-flex align-items-center " >
            ...
            <div className="col-8">
                <Select
                    ...
                    style={{ padding: 0 }}
                    value={OPTIONS_TEMPLATE.find((_op) => _op.value == templateId)}
                    onChange={({ value }) => {
                        if (!!value) {
                            setTemplateId(value)
                        } else {
            ...
            onClick={async e => {
              ...
              try {
                ...
                if (data?.scExportOrder?.success == 1) {
                  ...
                  onHide()
                  history.push(`/order-sales-person/history-export-file-approved-order`)
                } else {
...
export default injectIntl(ExportOrderDialog);