...
import { MANUAL_ORDER_SVG, NOTE_SVG } from './Constant';
import AuthorizationWrapper from '../../../../components/AuthorizationWrapper';
import { useToasts } from 'react-toast-notifications';
dayjs.extend(duration);


export default memo(({onApprovedManualOrder, onSetDataSmeNote, onSetSmeNote, onOpenConfirmDialog, dataSmeVariant, isSelected, setDataSelectedOrder,order }) => {
                    ...
                    <>
                        <AuthorizationWrapper keys={['order_sales_person_create_manual']}>
                            <Dropdown.Item className="mb-1 d-flex" onClick={async e => {
                                ...
                                window.open(`/order-sales-person/manual/${order?.id}?urlRedirect=${'/order-sales-person/approved-order'}`, '_blank')
                            }} >
                                {formatMessage({ defaultMessage: 'Sửa đơn hàng' })}
                            </Dropdown.Item>
                        </AuthorizationWrapper>
                        <AuthorizationWrapper keys={['order_approved']}>
                            <Dropdown.Item className="mb-1 d-flex" onClick={async () => await onApprovedManualOrder(order?.id)} >
                        ...
                        <AuthorizationWrapper keys={['order_sales_person_order_add_note']}>
                            {!order?.smeNote && (
                                <Dropdown.Item className="mb-1 d-flex" onClick={() => onSetDataSmeNote(order?.id)} >
                ...
                {!params?.is_old_order && (
                    <AuthorizationWrapper keys={['order_sales_person_create_manual']}>
                        ...
                        history.push({
                            ...
                            state: {isSale: true},
                            search: `?urlRedirect=${'/order-sales-person/approved-order'}`,
                        })    
    ...
    }, [location.pathname, params?.type, order])
                ...
                <div>
                        ...
                        {ttsExpired ? ttsExpired - dayjs().unix() > 0 ? 
                                ...
                                <div style={{color: 'F80D0D'}}>
                                    ...
                                    <TooltipWrapper note={<div><span>{formatMessage({defaultMessage: 'Còn lại'})}:</span>{Math.floor((ttsExpired - dayjs().unix()) / 3600)} {formatMessage({defaultMessage: 'giờ'})} {Math.floor(((ttsExpired - dayjs().unix()) % 3600) / 60)} {formatMessage({defaultMessage: 'phút'})}</div>}>
                                    ...
                                    <span className='text-primary'>
                                        {Math.floor((ttsExpired - dayjs().unix()) / 3600)} giờ {Math.floor(((ttsExpired - dayjs().unix()) % 3600) / 60)} phút
                                    </span></div> : 
                                <div>
    ...
    return (
        <Fragment>
            ...
            {
                order?.connector_channel_error ? (<tr style={{ background: 'rgba(254, 86, 41, 0.31)' }}>
                        <td colSpan={7}>
