...
export default memo(({errOrder, smeVariant, loadingSmeVariant, isGift, key, quantity_purchased, isBorder}) => {
    ...
    return (
        <div key={key} className='d-flex row w-100 m-0 p-2' style={!!isBorder ? { borderBottom: '0.5px solid #cbced4' } : {}}>
            ...
            <div className="col-11" style={{ verticalAlign: 'top', display: 'flex', flexDirection: 'row' }}>
                {loadingSmeVariant && (
                    <Skeleton style={{width: 60, height: 60, marginRight: 4, borderRadius: 4, minWidth: 60}} count={1}/>
                )}
                {!loadingSmeVariant && <div
                    style={{backgroundColor: '#F7F7FA', width: 60, height: 60, borderRadius: 4, overflow: 'hidden',minWidth: 60, cursor: 'pointer'}}
                    className='mr-6'
                >
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div className='d-flex w-100'>
                        ...
                        {smeVariant?.combo_items?.length > 0 && (
                            <span className='text-primary cursor-pointer ml-2' style={{ minWidth: 'fit-content' }} onClick={() => setDataCombo(smeVariant?.combo_items)}>
                                Combo
                            </span>)}
                    </div>
                </div>
