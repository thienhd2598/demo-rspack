...
export default memo(({
    ...
    allowDowload = false,
    multiple = false,
    isSmall, onMergeFrameSuccess, isValidate,
    isMedium = false,
    isSingle = false,
    disabled,
    required = true,
    ...
    const _reftemplate_image_url = useRef(template_image_url);
    const _refisMergeOption = useRef(isMergeOption);
    ...
    const _mergeFrame = async (_source, _frame, _merge_option) => {        
        setUploading(true);

        const bodyRequest = currentFrame?.is_static == 0
            ? {
                "productUrl": _source,
                "frameUrl": _frame,
                "shape": JSON.parse(currentFrame?.shape || "")
            } : {
                "originUrl": _source,
                "frameUrl": _frame,
                "option": Number(_merge_option) || 0
            }
        try {
            let res = await axios.post(currentFrame?.is_static == 0 ? process.env.REACT_APP_URL_MERGE_FRAME_DYNAMIC_PHOTO : process.env.REACT_APP_URL_MERGE_FRAME_PHOTO, bodyRequest, {
                isSubUser: user?.is_subuser,
                cancelToken: new CancelToken(function executor(c) {
            ...
            if (res.data.success) {
                setError(null)
                requestAnimationFrame(() => {
                    !!onMergeFrameSuccess && res.data.success && onMergeFrameSuccess(res.data.data.source, id)
                })
            } else {
                setError(formatMessage({ defaultMessage: 'Gặp lỗi trong quá trình thêm khung ảnh' }))
            }
    ...
    useEffect(() => {
        if (!!source && !!template_image_url) {
            if (!merged_image_url || (_reftemplate_image_url.current && _reftemplate_image_url.current != template_image_url) || _refisMergeOption.current != isMergeOption) {
                _reftemplate_image_url.current = template_image_url;
                _refisMergeOption.current = isMergeOption;

                _mergeFrame(source, template_image_url, isMergeOption)
            }
        }
    }, [source, template_image_url, merged_image_url, _reftemplate_image_url, _refisMergeOption, isMergeOption])
    ...
    useMemo(() => {
        if (!data) {
            ...
            _reftemplate_image_url.current = null
            _refisMergeOption.current = null
        }
        return () => {
            setPreview()
        }
    ...
    const view = (
        <div className="image-input my-4 mr-4 overlay" id="kt_image_4" style={{
            width: isSmall ? 60 : (isMedium ? 80 : 122), height: isSmall ? 60 : (isMedium ? 80 : 122),
        ...
        >
            {
                !source && !preview && <div style={{
                    ...
                    <i className='flaticon2-add-1' style={{ fontSize: isSmall ? 18 : (isMedium ? 24 : 28) }} ></i>
                    <span style={{ marginTop: 4, fontSize: isSmall ? 10 : (isMedium ? 12 : undefined) }} >{formatMessage({ defaultMessage: 'Tải ảnh lên' })}
            ...
            }
            {
                !!source && <div className="image-input-wrapper" style={{
                    ...
                    ...(isSmall ? { width: 56, height: 56 } : (isMedium ? { width: 80, height: 80 } : {}))
                }}></div>
            }
            {
                !!preview && !source && <img className="image-input-wrapper" style={isSmall ? { width: 56, height: 56 } : (isMedium ? { width: 80, height: 80 } : {})} src={preview} />
            }
